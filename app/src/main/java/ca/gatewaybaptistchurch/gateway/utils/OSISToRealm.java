...
public class OSISToRealm extends AsyncTask<Void, Void, Bible> {
	private static final String TAG = OSISToRealm.class.getSimpleName();
	...
	protected Bible doInBackground(Void... params) {
		try {
			...
			Document document = documentBuilder.parse(GatewayApplication.getContext().getResources().openRawResource(R.raw.esv));
			NodeList infoNodes = document.getElementsByTagName("work");
	...
	private Bible buildBible(Realm realm, Document document, Node bibleInfoNode) {
		...
		NodeList bookNodes = document.getElementsByTagName("div");
